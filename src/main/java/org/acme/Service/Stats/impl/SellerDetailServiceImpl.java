        ...
        for (Order order : inPeriod) {
            ...
            if (order.getArticles() != null) {
                for (OrderItem item : order.getArticles()) {
                    itemsSoldCount = itemsSoldCount.add(item.getQuantityOrdered());
                    BigDecimal lineCa = item.getPrice().multiply(item.getQuantityOrdered());
                    itemQty.merge(item.getName(), item.getQuantityOrdered(), BigDecimal::add);
